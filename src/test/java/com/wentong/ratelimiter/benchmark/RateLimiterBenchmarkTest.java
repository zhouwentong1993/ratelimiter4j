package com.wentong.ratelimiter.benchmark;

import com.wentong.ratelimiter.DistributedUrlRateLimiter;
import com.wentong.ratelimiter.Interceptor.RateLimiterInterceptorChain;
import com.wentong.ratelimiter.MemoryUrlRateLimiter;
import com.wentong.ratelimiter.UrlRateLimiter;
import com.wentong.ratelimiter.exception.InternalErrorException;
import com.wentong.ratelimiter.exception.InvalidUrlException;
import com.wentong.ratelimiter.exception.OverloadException;
import com.wentong.ratelimiter.redis.DefaultJedisPoolConfig;
...
import com.wentong.ratelimiter.utils.UrlUtils;
...
public class RateLimiterBenchmarkTest {
    ...
    private static void benchmark(boolean isDistributed) {
        ...
        for (Case c : cases) {
            ...
            if (isDistributed) {
                ...
                ratelimiter = new MemoryUrlRateLimiter(rule, new RateLimiterInterceptorChain());
            }
    ...
    private static void print(Case c, List<Long> responseTimes, long wallCostTime) {
        ...
        String report = String.format("%.5f", time9999) + "," +
                String.format("%.5f", time999) + "," +
                String.format("%.5f", time99) + "," +
                String.format("%.5f", time90) + "," +
                String.format("%.5f", timeMax) + "," +
                String.format("%.5f", avgTime) + "," +
                (int) ftps + "" +
                "wall time:" + wallCostTime / 1000000f + "; ";
